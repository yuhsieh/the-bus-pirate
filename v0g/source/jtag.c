...
void jtagProcess(unsigned char cmd, unsigned char val){
	...
	switch(cmd){
		...
		case CMD_MACRO:
			switch(val){
				case 0://jtag reset
					bpWstring(" 0.Macro menu\x0D\x0A 1.Reset chain\x0D\x0A 2.Probe chain\x0D\x0A 3.XSVF player\x0D\x0A");
					break;
				case 1://jtag reset
					jtagReset();//reset
