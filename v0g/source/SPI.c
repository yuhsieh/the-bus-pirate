...
void spiProcess(unsigned char cmd, unsigned char val){
	...
	switch(cmd){
		...
		case CMD_PRESETUP:
			...
			bpWmessage(MSG_OPT_SPI_SPEED);
			spiSettings.speed=(bpUserNumberPrompt(1, 4, 1)-1);

			//bpWstring("Clock polarity:\x0D\x0A 1. Idle low *default\x0D\x0A 2. Idle high\x0D\x0A");
			bpWmessage(MSG_OPT_SPI_CKP);
		...
		case CMD_MACRO:
			switch(val){
				case 0:
					//bpWstring(" 0.Macro menu\x0D\x0A 1.SPI bus sniffer (filter with CS pin)\x0D\x0A 2.SPI bus sniffer (all traffic)\x0D\x0A");
					bpWstring(" 0.Macro menu\x0D\x0A 1.SPI bus sniffer\x0D\x0A");
					break;
				case 1:
					bpWstring("Sniff when:\x0D\x0A 1. CS low\x0D\x0A 2. CS high\x0D\x0A 3. All traffic\x0D\x0A");
...
void spiSniffer(unsigned char csState){
	...
	bpWline("SPI BUS SNIFFER, PRESS ANY KEY TO EXIT");
	spiDisable();
	spiSlaveSetup();
	while(1){
		...
		if(SPI1STATbits.SPIROV==1 || SPI2STATbits.SPIROV==1 ){//we weren't fast enough, buffer overflow
			bpWline("BUFFER OVERFLOW, DROPPED A BYTE.\x0D\x0A BUS TOO FAST, TRY RAW OUTPUT DISPLAY MODE.");	
...
//clear old SPI settings first
	SPI1CON1 = (SPIspeed[spiSettings.speed]); // CKE (output edge) active to idle, CKP idle low, SMP data sampled middle of output time.
	...
	SPI2CON1 = (SPIspeed[spiSettings.speed]); // CKE (output edge) active to idle, CKP idle low, SMP data sampled middle of output time.
