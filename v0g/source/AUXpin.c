...
unsigned int getINTvalue(unsigned int i);
...
void bpPWM(void){
	...
	if(AUXmode==AUX_PWM){
		...
		bpWline("PWM DISABLED");
		AUXmode=AUX_IO;
		return;
	}

	//calculate frequency:
	bpWline("1KHz-4,000KHz frequency generator/PWM (beta)");
	bpWstring("FREQUENCY in KHz ");
	...
	bpWstring("PWM ACTIVE\x0D\x0A");
	AUXmode=AUX_PWM;

}

unsigned int getINTvalue(unsigned int limit){
	#define INT_VALUE_BUFFER	7
	unsigned char c;
	unsigned char terminalInput[INT_VALUE_BUFFER];
	unsigned char b=0,j;
	unsigned int i=0;
		...
		if(UART1RXRdy()){
			...
			switch(c){
				...
				case 0x0D:	
					...
					if(b==0){//no command
						bpWstring(" ERROR, ENTER A DECIMAL INTEGER>");
						break;
					}
	
					for(j=0;j<b;j++){
						i*=10;
						i+=(terminalInput[j]-0x30);
					}
					bpBR;
					return i;
...
void bpFreq(void){
	...
	if(AUXmode==AUX_PWM){
		bpWline("PWM ACTIVE: DISABLE PWM");
		return;
	}

	bpWstring("FREQUENCY COUNT ON AUX: ");
//setup timer
