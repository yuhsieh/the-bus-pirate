...
void processSyntaxString(unsigned int totalBytes, unsigned char *commandArr){
	static unsigned int currentByte,repeatVal, writeVal, macroVal;
		...
		switch(command){
			...
			case CMD_AUXIN://aux to input, read
				...
				case CMD_ADC://do an adc reading
					bpWstring("VOLTAGE PROBE: ");	
					bpADCprobe();
					bpWline("VOLTS");
					break;
			#endif
			case CMD_BIT_CLK:
				//pass i by reference because it needs to be updated
				repeatVal=bpGetRepeatVal(totalBytes, &currentByte, commandArr);
				bpProcess(CMD_BIT_CLK, repeatVal,0);	
				break;
			case CMD_READ:
				//pass i by reference because it needs to be updated
				repeatVal=bpGetRepeatVal(totalBytes, &currentByte, commandArr);
				if(repeatVal==1){
					bpProcess(CMD_READ, 0,0);
				}else{
					bpProcess(CMD_READBULK, repeatVal,0);
				}
				break;
			case CMD_DELAY://delay
				...
				bpWstring("DELAY ");
				bpWbyte(repeatVal);
				bpWstring("uS\x0D\x0A");
				bpDelayUS(repeatVal);
				break;		
			case CMD_MACRO://macro
				//loop forwards and grab macro
				if(currentByte==totalBytes){error++; break;}//don't overrun the array
				currentByte++;//increment the pointer
				macroVal=bpGetASCIInumber(totalBytes, &currentByte, commandArr); //bpGetMacroVal(&error, totalBytes, &currentByte, commandArr);
				currentByte++;
				//check if number ends with ), check for unexpected end of sequence
				if(currentByte==totalBytes || commandArr[currentByte]!=')'){
					currentByte--;
					error++;
					break;
				}
				//send integer to the active function, let it deal with macros...
				bpProcess(CMD_MACRO,macroVal,0);//process the macro
				break;
			case CMD_WRITE://deal with actual numbers here 
				writeVal=bpGetASCIInumber(totalBytes, &currentByte, commandArr);

				//pass i by reference because it needs to be updated
				repeatVal=bpGetRepeatVal(totalBytes, &currentByte, commandArr);

				if(repeatVal==1){
					bpProcess(CMD_WRITE, writeVal,0); //write to the bus
				}else{
					bpProcess(CMD_WRITE_BULK, writeVal,repeatVal); //write to the bus
				}
				break;
			default:
				//send command to library
				bpProcess(command,0,0);
		}//switch
