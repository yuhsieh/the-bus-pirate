...
void DS1wireProcess(unsigned char cmd, unsigned char val){
	...
	switch(cmd){
		...
		case CMD_BIT_DATL:
			...
			bpWstring(" *next clock (^) will also have this value\x0D\x0A");
		...
		case CMD_MACRO:
			if(val>0 && val<51){
				val--;//adjust down one for roster array index
				if(val>=OWroster.num){//no device #X on the bus, try ROM SEARCH (0xF0)
				...
				bpWdec(val+1);
				bpWstring(": ");
				for(j=0;j<8;j++){bpWbyte(OWroster.dev[val].id[j]); bpWstring(" "); OWWriteByte(OWroster.dev[val].id[j]);} //write address
				bpWline(" ");
				return;
			}
			switch(val){
				case 0://menu
					...
					bpWstring("1WIRE ROM COMMAND MACROs:\x0D\x0A 51.READ ROM (0x33) *for single device bus\x0D\x0A 85.MATCH ROM (0x55) *followed by 64bit address\x0D\x0A 204.SKIP ROM (0xCC) *followed by command\x0D\x0A 236.ALARM SEARCH (0xEC)\x0D\x0A 240.SEARCH ROM (0xF0)\x0D\x0A");
					break;
				//1WIRE ROM COMMANDS
