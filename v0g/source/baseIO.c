...
//get a response value from 1-i, repeats until valid
/*unsigned char bpGetMenuReply(unsigned char i){
	unsigned char c;
	...
	while(1){
		...
		c=UART1RX();
		UART1TX(c);//echo back
		c-=0x31;//adjust from ascii	
		bpWstring("\x0D\x0A");
		if(c<i)break;//total options....
		bpWstring("SYNTAX ERROR, TRY AGAIN>");
	}
	return c+1;
}*/
...
/*
	...
	c=10000;
    if(b/c){UART1TX(b/c + '0'); k=1;}
    b = b - (b/c)*c;

	c=1000;
    if(k==1 || b/c){UART1TX(b/c + '0'); k=1;}
    b = b - (b/c)*c;
...
/*
    b = c;
    if(b/100){UART1TX(b/100 + '0');k=1;}
    b = b - (b/100)*100;

    if(k==1 || b/10){UART1TX(b/10 + '0');}
    b = b - (b/10)*10;
