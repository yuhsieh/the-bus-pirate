...
void kbWriteBit(unsigned char c);
...
void kbProcess(unsigned char cmd, unsigned char val){
	...
	switch(cmd){
		...
		case CMD_WRITE:
			...
			bpWbyte(val);//write value to term
			c=kbWriteByte(val);//send to bus
			if(c==0){//ack bit
				bpWstring(" GOT ACK");		//success
			}else{
				bpWstring(" ERROR, NO ACK");
			}
			bpWBR;
			break;
		case CMD_MACRO:
...
void kbScancodeResults(unsigned char c){
	switch(c){
		case 0:
			...
			break;
		case 1:
			bpWstring(" *startbit error");				
			break;
		case 2:
			bpWbyte(kbScancode.code);
			bpWstring(" *parity error");				
			break;
		case 3:
			bpWbyte(kbScancode.code);
			bpWstring(" *stopbit error");				
			break;
		case 0xff://no data
			bpWstring(" NONE");				
			break;
		default:
			bpWstring(" UNKNOWN ERROR");				
			break;
	}
}
...
unsigned char kbReadBit(void){
	...
	while(KBCLK==1);//wait for clock High->LOW (ACTIVE)
	j=KBDIO; //read the data pin
	while (KBCLK==0);//wait for clock low->HIGH (IDLE)
	return j;
}

//reads scancode into kbScancode structure
unsigned char kbReadCode(void){
	unsigned char i, par=0,c=0;
	...
	kbScancode.paritybit=kbReadBit();
	...
	//get stopbit
	kbScancode.stopbit=kbReadBit();//stopbit should be 1
...
unsigned char kbReadByte(void){
	...
	//while(1){//for debugging
		if(KBCLK==0){//check clock status
			...
			if(c==0)break;//success, break the loop
		}
		bpDelayUS(5);//delay
	}
...
void kbWriteBit(unsigned char c){
	while(KBCLK==0);//wait for low-high transition
	KBDIO_TRIS=c;
	while(KBCLK==1);//wait for high-low transition
}	//data now entered, you have a tiny bit of time before next bit

//@c - byte to send
//@returns - ack bit
unsigned char kbWriteByte(unsigned char c){
	unsigned char i,par=0;
	...
	for(i=0;i<8;i++){
		if((c&1)==1){
			kbWriteBit(1);//send LSB first
			par^=1;//track parity bit
		}else{
			kbWriteBit(0);//send LSB first	
		}		
	...
	kbWriteBit(par);//send parity bit
	...
	kbReadBit();//junk bit (stopbit/high)
	i=kbReadBit();//ack bit

	KBCLK_TRIS=0;//set clock low again to halt data
	KBCLK=0;
	//bpWstring(" PARITY:");		//success
	//bpWbyte(par);
