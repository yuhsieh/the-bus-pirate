...
struct _bpConfig bpConfig; //holds persistant bus pirate settings (see base.h)
...
void Initialize(void){
	...
	//put startup vales in config (do first)
	bpConfig.termSpeed=8;//default PC side port speed, startup in 115200, or saved state (later)....
	bpConfig.displayMode=HEX;

	bpInit();//put startup vales in config (do first)clean up, exit in HI-Z
		
	InitializeUART1(); //init the PC side serial port (needs busPirateInit() first for speed setting)
